...
def get_hash(repo_name: str, chart_name: str) -> str:
    ...
    for line in cp.stderr.split("\n"):
        ...
        return line.strip()
    raise RuntimeError(
        f"failed to get the hash for {repo_name}/{chart_name}:\n{cp.stderr}"
    )


def get_charts():
    return json.loads(
        subprocess.check_output(["nix", "eval", ".#chartsMetadata", "--json"])
    )
    ...
    if commit:
        ...
        subprocess.run(
            [
                ...
                "commit",
                "-m",
                f"{repo_name}/{chart_name}: update to {remote_version}",
            ],
            check=True,
        )
...
def init(
    ...
    name: str,
    commit: bool = typer.Option(False),
    bogus_version: bool = typer.Option(False),
):
    repo_name, chart_name = name.split("/")
    charts = get_charts()
    ...
    local_chart = charts[repo_name][chart_name]
    update_one_chart(
        repo_name, chart_name, local_chart, commit=False, fail_on_fetch=True
    )
    ...
    if commit:
        subprocess.run(
            [
                ...
                "commit",
                "-m",
                f"{repo_name}/{chart_name}: init at {local_chart['version']}",
            ],
            check=True,
        )
